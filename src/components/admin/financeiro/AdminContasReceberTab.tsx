...
export const AdminContasReceberTab = () => {
  ...
  const kpi = useMemo(() => {
    ...
    const received = installments.reduce((sum, it) => sum + safeNum(it.received_amount), 0);
  ...
  const recomputeInvoiceStatus = async (invoiceId: string) => {
    ...
    const received = inst.reduce((s, i) => s + safeNum(i.received_amount), 0);

    let status: InvoiceStatus = "aberto";
    if (open <= 0 && inst.length > 0) status = "pago";
    else if (received > 0 && open > 0) status = "parcial";

    const { error } = await supabase.from("ar_invoices").update({ status }).eq("id", invoiceId);
  ...
  const cancelInvoice = async (inv: Invoice) => {
    const { error: invErr } = await supabase.from("ar_invoices").update({ status: "cancelado" }).eq("id", inv.id);
    if (invErr) {
      toast({ title: "Erro", description: invErr.message, variant: "destructive" });
      return;
    }

    const { error: instErr } = await supabase.from("ar_installments").update({ status: "cancelado" }).eq("invoice_id", inv.id);
    if (instErr) {
                                    ...
                                    <div className="flex items-start justify-between gap-2">
                                      <div>
                                        ...
                                        <p className="text-xs text-muted-foreground">
                                          ...
                                          <span className="text-green-600">{formatCurrencyBRL(safeNum(it.received_amount))}</span> • Aberto:{" "}

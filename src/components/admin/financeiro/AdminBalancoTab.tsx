  ...
  const { data: rawMaterials = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("raw_materials")
        .select("id, name, current_quantity, avg_cost_per_unit, unit, is_active")
        .eq("is_active", true);
  ...
  const computed = useMemo(() => {
    ...
    const rawInventoryValue = sum(
      (rawMaterials as any[]).map(
        (m) => safeNum(m.current_quantity) * safeNum(m.avg_cost_per_unit)
      )
    );
    ...
    const rawTop = (rawMaterials as any[])
      .map((m) => ({
        id: m.id,
        name: m.name,
        value: safeNum(m.current_quantity) * safeNum(m.avg_cost_per_unit),
        qty: safeNum(m.current_quantity),
        unit: String(m.unit ?? ""),
        avg: safeNum(m.avg_cost_per_unit),
      }))
